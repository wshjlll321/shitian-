...
export function AdminEditToolbar({
  ...
  return (
    <div className="sticky top-3 z-20 mt-5 flex justify-start md:justify-end">
      <div className="flex w-full flex-col gap-3 border border-carbon-black/12 border-l-2 border-l-aviation-orange bg-surface-porcelain/95 p-3 shadow-[0_18px_45px_-34px_rgba(17,17,17,0.5)] backdrop-blur md:w-auto md:flex-row md:items-center">
        <div className="min-w-0 md:max-w-56">
          <p className="text-[12px] font-medium text-carbon-black">{title}</p>
          <p className="mt-0.5 text-[11px] leading-5 text-carbon-black/45">{description}</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {viewHref ? (
            <Link
              href={viewHref}
              target="_blank"
              className="inline-flex min-h-9 items-center border border-carbon-black/15 bg-white px-3 text-[12px] text-carbon-black/60 transition hover:border-aviation-orange hover:text-aviation-orange"
            >
              前台查看
            </Link>
          ) : null}
